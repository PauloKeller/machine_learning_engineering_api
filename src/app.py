...
def write_real_data(data, prediction):
    ...
    bucket_name = 'paulo-fiap-ds-mlops'
    s3_path = 'lcredit_score_quantum_finance_real_data_10dtsr'
...
def input_metrics(data, prediction):
    ...
    for key, value in data.items():
        ...
        MetricData = [
            {
                'MetricName': 'Credit Score Feature',
                'Value': 1,
                'Unit': 'Count',
...
def prepare_payload(data):
    ...
    data_processed.append(int(data["Changed_Credit_Limit"]))
    data_processed.append(int(data["Num_Credit_Inquiries"]))
    data_processed.append(int(data["Credit_Utilization_Ratio"]))
    data_processed.append(int(data["Total_EMI_per_month"]))
